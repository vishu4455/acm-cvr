...
interface Ping { x: number; y: number; r: number; maxR: number; alpha: number; }
...
export function CustomCursor() {
  ...
  useEffect(() => {
    ...
    function sparkColor() {
      ...
      if (m === 'button' || m === 'canvas-hotspot') return NODE;
      if (m === 'text') return TRACE;
      if (m === 'disabled') return IDLE;
      return PULSE;
    }

    function spawnSparks(x: number, y: number, intensity: number) {
      ...
      if (sparks.length > 140) sparks = sparks.slice(sparks.length - 140);
    }
  ...
  const modeStyles: Record<string, { dot: string; ring: string }> = {
  ...
  return (
    <>
      ...
      <div
        ref={dotRef}
        className={`fixed left-0 top-0 h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2
          rounded-full pointer-events-none z-[9999] transition-colors duration-150 ${style.dot}`}
      />
      <div
        ref={ringRef}
        className={`fixed left-0 top-0 h-7 w-7 -translate-x-1/2 -translate-y-1/2
