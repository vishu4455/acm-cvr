...
export function TeamGraph() {
    ...
    const layout = useMemo(() => computeGraphLayout(nodeInputs, edges, WIDTH, HEIGHT), [nodeInputs, edges]);
    ...
    return (_jsx("div", { className: "hidden md:block overflow-x-auto", children: _jsxs("svg", { viewBox: `0 0 ${WIDTH} ${HEIGHT}`, className: "w-full h-auto", role: "group", "aria-label": "Team network", children: [_jsx("g", { opacity: 0.9, children: edges.map((edge, i) => {
                        ...
                        return (_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: active ? '#5FA8B8' : '#2A3438', strokeWidth: active ? 1.5 : 1, style: { transition: 'stroke 200ms ease-out' } }, i));
                    }) }), (() => {
                    const root = positionById.get(facultyCoordinator.id);
                    if (!root)
                        return null;
                    return (_jsx("circle", { cx: root.x, cy: root.y, r: root.radius, fill: "#3A3B3F", stroke: "#55575D", strokeWidth: 2 }));
                })(), allMembers.map((member) => {
                    const pos = positionById.get(member.id);
                    if (!pos)
                        return null;
                    return (_jsx(TeamNode, { member: member, x: pos.x, y: pos.y, radius: pos.radius, dimmed: connectedIds ? !connectedIds.has(member.id) : false, onHover: setHoveredId, onSelect: openMemberProfile }, member.id));
                })] }) }));
}
