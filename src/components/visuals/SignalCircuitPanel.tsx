 ...
 */
export function SignalCircuitPanel({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function distToPolyline(px: number, py: number, points: { x: number; y: number }[]) {
      let m = Infinity;
    ...
    function pointAtT(points: { x: number; y: number }[], t: number) {
      const lens: number[] = [];
      let total = 0;
      for (let i = 0; i < points.length - 1; i++) {
    ...
    // ---------- model ----------
    interface Pin { px: number; py: number; sx: number; sy: number; dx: number; dy: number; }
    interface Trace {
    ...
    function buildTraces() {
      ...
      const clampX = (v: number) => Math.max(margin, Math.min(W - margin, v));
      const clampY = (v: number) => Math.max(margin, Math.min(H - margin, v));
      ...
      pins.forEach((pin) => {
        ...
        const points: { x: number; y: number }[] = [{ x: pin.px, y: pin.py }, { x: pin.sx, y: pin.sy }];

        const reach = 40 + rand() * Math.max(60, minDim * 0.22);
        let x = pin.sx, y = pin.sy;

        const leg1 = reach * (0.45 + rand() * 0.35);
        if (mainDir === 'h') { x = pin.sx + pin.dx * leg1; } else { y = pin.sy + pin.dy * leg1; }
        points.push({ x: clampX(x), y: clampY(y) });

        if (rand() < 0.8) {
          const perp = rand() < 0.5 ? -1 : 1;
          const turnAmt = (10 + rand() * 40) * perp;
          if (mainDir === 'h') { y += turnAmt; } else { x += turnAmt; }
          points.push({ x: clampX(x), y: clampY(y) });

          const leg2 = reach * 0.5;
          if (mainDir === 'h') { x += pin.dx * leg2; } else { y += pin.dy * leg2; }
          points.push({ x: clampX(x), y: clampY(y) });
        }

        if (rand() < 0.3) {
          if (mainDir === 'h') { y += (rand() < 0.5 ? -1 : 1) * (6 + rand() * 18); }
          else { x += (rand() < 0.5 ? -1 : 1) * (6 + rand() * 18); }
          points.push({ x: clampX(x), y: clampY(y) });
        }
    ...
    function traceActivation(tr: Trace) {
      let target = 0;
      if (mouse.active) {
    ...
    function drawTrace(tr: Trace, t: number) {
      const pts = tr.points;
  ...
  return (
    <div ref={containerRef} className={`relative aspect-square overflow-hidden rounded-md ${className}`}>
